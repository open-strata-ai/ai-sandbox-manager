...
This ADR solidifies "RuntimeClass injection with `ai-provisioning-engine`" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
