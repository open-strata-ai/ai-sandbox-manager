# ADR-0001: E2B self-hosted vs cloud
...
E2B is cloud-based (commercial) by default. Does the enterprise intranet need to self-host the E2B control plane? Affects `egress` compliance (§4.4.6).
...
This ADR solidifies "E2B self-hosted vs. cloud" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
